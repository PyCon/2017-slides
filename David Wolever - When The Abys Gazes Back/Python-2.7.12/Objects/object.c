...
PyObject_RichCompare(PyObject *v, PyObject *w, int op)
{
    ...
    /**
     ** What's PyInstance_Check(...)?
     ** It distinguishes between "old style" classes and "new style" classes:
     ** instances of old-style classes are called "instances", and instances of
     ** new-style classes are called "objects".
     ** 
     ** What are "old-style" and "new-style" classes?
     ** Python 2.1 introduced a new object model that was incompatible with the
     ** previous object model. The details are out of scope of this talk... but
     ** in brief: if you've ever left the `object` out of a class definition:
     ** 
     **     class Foo: (instead of `class Foo(object):`)
     **         pass
     ** 
     ** You've (likely accidentally) created an old-style class.
     ** 
     ** Suggested homework: explain exactly why `@property` doesn't work with
     ** old style classes.
     **/
    ...
    if (v->ob_type == w->ob_type && !PyInstance_Check(v)) {
        ...
        /**
         ** Notice that struct attributes are accessed using `obj->pfx_attr`.
         ** Structs are *sorta kinda* like objects, and it's customary in C to
         ** have a common prefix on structure attributes, possibly to help the
         ** reader understand what types we're dealing with, possibly to make
         ** autocomplete easier in days before IDEs, and possibly other reasons
         ** too!
         ** 
         ** In this case, the `ob_` prefix is used for members of the PyObject
         ** type (PyObject is *sorta kinda* like the `object` base class), and
         ** the `tp_` prefix is for members of the PyTypeObject type (which
         ** is *sorta kinda* similar to the `type` class), and it implements
         ** __init__, __eq__, etc.
         **/

        richcmpfunc frich = v->ob_type->tp_richcompare;
        if (frich != NULL) {

            /**
             ** What is a "rich comparison"?
             **
             ** Before Python 2.1 (ie, old-style classes), objects could
             ** only define a __cmp__ method which returning -1, 0, or 1
             ** (see the cmp(a, b) builtin).
             **
             ** The rich comparisons introduced with new-style classes let
             ** objects define different methods for ==, <=, <, etc: __eq__,
             ** __le__, __lt__, etc.
             **
             ** See also: PEP 207
             **/
            ...
            /**
             ** It's easy to miss, but here's where the comparison function
             ** is called!
             **
             ** To follow what's happening here, notice that:
             **
             **     frich = v->ob_type->tp_richcompare
             **
             ** And `v->ob_type` is `PyString_Type`, so:
             **
             **     frich = PyString_Type->tp_richcompare
             **
             ** Which means we need to find `PyString_Type->tp_richcompare`.
             **
             ** Jump to: PyString_Type
             **/
...
PyObject_RichCompareBool(PyObject *v, PyObject *w, int op)
{
    ...
    /**
     ** Remember string interning? The same thing applies here!
     ** The 'x' in both 'x' and ('x', ) will be the same string in memory,
     ** so `v == w` will be true!
     **/
    if (v == w) {
        if (op == Py_EQ)
            return 1; /** <--- our trace ends here! **/
        else if (op == Py_NE)
            return 0;
    }
