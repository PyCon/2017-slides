...
PySequence_Contains(PyObject *seq, PyObject *ob)
{
    ...
    /**
     ** Recall that `seq->obj_type` will be the PyTypeObject of the type,
     ** which in this case will be `PyTuple_Type`.
     **/

    if (PyType_HasFeature(seq->ob_type, Py_TPFLAGS_HAVE_SEQUENCE_IN)) {

        /**
         ** The call to `PyType_HasFeature` is difficult to trace through, so
         ** we won't. But just based on the names, it's reasonable to guess
         ** that this is checking whether the type supports membership with
         ** `in`, which tuples obviously do!
         **/

        /**
         ** To expand the next two lines:
         **
         **   sqm = seq->ob_type->tp_as_sequence;
         **
         ** Becomes:
         **
         **   sqm = PyTuple_Type->tp_as_sequence;
         **
         ** And so:
         **
         **   sqm->sq_contains
