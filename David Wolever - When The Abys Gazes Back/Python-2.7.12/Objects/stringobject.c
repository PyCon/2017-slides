...
string_richcompare(PyStringObject *a, PyStringObject *b, int op)
{
    ...
    /**
     ** Now this is where something special happens.
     ** Remember how both previous calls to LOAD_CONST had the same argument?
     **
     **                          v---- this is the argument
     **       0 LOAD_CONST       1 ('x')
     **       3 LOAD_CONST       1 ('x')
     **       6 COMPARE_OP       2 (==)
     **       9 RETURN_VALUE 
     **
     ** This happens because, when Python loads a module, it de-duplicates
     ** some of the strings through a process called interning*.
     **
     ** For example:
     **
     **     >>> foo = "abc"
     ...
     **     True
     **
     ** The string "abc" only exists once in memory, and both variables
     ** will reference that same place (this is possible because strings in
     ** Python are immutable, and one of the reasons that languages with
     ** mutable strings - like Ruby - also have immutable string-like "symbol"
     ** types).
     **
     ** *: The rules for string interning are complex and fascinating. There's
    ...
    if (a == b) {
        ...
        case Py_EQ:case Py_LE:case Py_GE:
            result = Py_True; /** <--- our trace ends here! **/
            goto out;
